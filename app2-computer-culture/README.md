...
- fork：创建一个项目的独立副本，通常用于贡献代码或自定义开发。就像是一条河流上分叉出一个分支，因而得名 fork（分支）。`fork()`也是 POSIX 标准中的一个函数，用于创建一个新进程。
...
- AFK：“Away From Keyboard”（离开键盘），表示聊天中暂时离开。这一俚语并不是严格意义上的计算机俚语，也多见于计算机社区外。
...
- cf：“Cloudflare”的缩写。CloudFlare 是一个全球性的云平台。
- ms/m\$：“Microsoft”（微软）的缩写。“m$”这一缩写经常用于讽刺微软见钱眼开，做出利益导向的决策。
- ff：“Firefox”（火狐）的缩写，久负盛名的开源浏览器。
